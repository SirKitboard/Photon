  ...
  namespace sssf {
    namespace win32 {
      ...
      void WindowsInput::processInput(Game* game) {
        WindowsOS* os = static_cast<WindowsOS*>(game->getOS());
        const WINDOWINFO& wi = os->getWindowInfo();
      ...
      /*
          ...
          */
      void WindowsInput::updateCursorPosition(Game* game, const WINDOWINFO& wi, gui::Cursor* cursor) {
        ...
        mousePoint.x = util::clamp(mousePoint.x, 0, graphics->getScreenWidth() - 1);
        mousePoint.y = util::clamp(mousePoint.y, 0, graphics->getScreenHeight() - 1);

        cursor->setPosition(mousePoint.x, mousePoint.y);
      }
