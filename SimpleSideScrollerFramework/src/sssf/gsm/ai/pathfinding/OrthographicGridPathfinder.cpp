		...
		namespace gsm {
			...
			namespace ai {
				namespace pathfinding {
					OrthographicGridPathfinder::OrthographicGridPathfinder(game::Game *initGame)
					{
						game = initGame;

						// WE'LL DETERMINE THE NUMBER OF ROWS AND COLUMNS BASED ON TEH FIRST
						// COLLIDABLE TILED LAYER WE FIND
						world::World *world = &(game->getGSM().getWorld());
						...
						float endX, float endY)
					{
						...
						state::GameStateManager *gsm = &(game->getGSM());
						world::World *world = &(gsm->getWorld());
					...
					int OrthographicGridPathfinder::calculateH(PathNode start, PathNode *end)
					{
						...
						int numDiagonals = std::min(colDiff, rowDiff);
					...
					void OrthographicGridPathfinder::updatePath(sprite::AnimatedSprite *sprite)
					{
...
//							}
					...
					{
						physics::AABB *spriteAABB = &(sprite->getBoundingVolume());
